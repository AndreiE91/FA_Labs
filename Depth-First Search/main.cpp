    ...
    do{
        ...
        if(strcmp(token, "exit") == 0 || strcmp(token, "quit") == 0 || strcmp(token, "q") == 0){
            ...
            validCommand = 1;
                ...
                strcmp(token, "path") == 0 || strcmp(token, "dfs") == 0 || strcmp(token, "dfs_step") == 0 || strcmp(token, "dfs_tree") == 0){
            ...
            else if (strcmp(token, "dfs") == 0) {
                c->command = COMMAND_DFS;
            }
            else if (strcmp(token, "dfs_step") == 0) {
                c->command = COMMAND_DFS_STEP;
            }
            else if (strcmp(token, "dfs_tree") == 0) {
                c->command = COMMAND_DFS_TREE;
            }
...
int main()
{
    ...
    for(;;){
        ...
        showErrors(errors);
        if(cmd.command == COMMAND_BFS_TREE || cmd.command == COMMAND_DFS_TREE){
        ...
        if(cmd.command == COMMAND_EXIT){
        ...
        else if (cmd.command == COMMAND_DFS || cmd.command == COMMAND_DFS_STEP || cmd.command == COMMAND_DFS_TREE) {
            grid2 = grid;
            grid_to_graph(&grid, &graph);
            //find the starting node
            int i;
            ...
            if (s == NULL) {
                ...
                if (cmd.command == COMMAND_DFS || cmd.command == COMMAND_DFS_TREE) {
                    for (i = 0; i < graph.nrNodes; ++i) {
                        if (graph.v[i]->color == COLOR_BLACK) {
                            Point p1;
                            p1 = graph.v[i]->position;
                ...
                else if (cmd.command == COMMAND_DFS_STEP) {
                    int newLayer = 1;
                    int crtDist = 0;
                    displayGrid(&grid2, cmd.command);
                    while (newLayer) {
